...
void create_json_array(Report info) {
	...
	json_object_set_new(obj, "desc", json_string(info.desc));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id", "name", "ticket", "desc" };
    char** infovar[] = {&info->id,&info->name,&info->ticket,&info->desc};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
	...
	dbres |= sqlite3_bind_text(stmt, 4, info.desc, -1, SQLITE_TRANSIENT);   
